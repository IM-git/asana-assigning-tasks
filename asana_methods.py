...
class AsanaMethods:
    ...
    def get_list_tasks(self, section_gid=AsanaData.SECTION_GID.value):
        result = self.client.tasks.get_tasks_for_section(section_gid,
                                                         opt_pretty=True)
        list_with_values = EnumerationTools().enum_cycle(result)
        return list_with_values

    def add_already_created_task_to_addition_project(self, task_id, project_id):
        task = self.client.tasks.get_task(task_id)
        project = self.client.projects.get_project(project_id)
        self.client.tasks.add_project(task['gid'], project=project['gid'])


if __name__ == '__main__':
    asana_trigger = AsanaMethods()
    print("### _API ASANA_ ###")
